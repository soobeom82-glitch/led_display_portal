...
제외하고 화면을 좌우로 스와이프해 어제, 오늘, 내일의 회의 시간과 회의실을 탐색할 수 있습니다.
참여를 거절한 일정과 제목에 `휴가`가 포함된 일정은 표시하지 않습니다.
일정 행을 누르고 있는 동안에는 해당 일정의 제목을 임시로 표시합니다.
Google Meet 일정은 회의실 대신 `화상`, 실제 회의실도 있으면 `B7-R11 (8) · 화상`으로 표시합니다.
실제 다음 회의를 크게 강조하고 30분 전부터 시작까지 남은 분을 시간 옆에 표시합니다.
3분 이하에서는 남은 분이 빨간색으로 1초마다 점멸합니다. 우측 상단에는 Vercel이 마지막
캘린더 스냅샷을 받은 시각을 표시합니다.
