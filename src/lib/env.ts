...
export function getAppEnv() {
  ...
  const publicKeyPem = process.env.TESLA_PUBLIC_KEY_PEM?.trim() ?? "";
  const kvReady =
  ...
  return {
    ...
    teslaPartnerAuthBaseUrl:
      process.env.TESLA_PARTNER_AUTH_BASE_URL?.trim() ??
    ...
    teslaPartnerScope: process.env.TESLA_PARTNER_SCOPE?.trim() ?? "",
    ...
    hasTeslaPublicKey: publicKeyPem.length > 0,
  };
}
